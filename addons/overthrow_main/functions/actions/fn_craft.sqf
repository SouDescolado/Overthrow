...
if (_def isNotEqualTo []) then {
    ...
    if !(isNull _container) then {
...

        {
            ...
            if !(_good) exitWith {
                _err = true;
                "Required ingredients not in closest ammobox" call OT_fnc_notifyMinor;
            };
        } forEach (_recipe);
