...
if (_civ getVariable ["carshop", false]) then {
    _canSellDrugs = true;
    _canRecruit = false;
    _canBuyVehicles = true;
};
if (_civ getVariable ["harbor", false]) then {
    _canSellDrugs = true;
    _canRecruit = false;
    _canBuyBoats = true;
};
if (_civ getVariable ["gundealer", false]) then {
    _canSellDrugs = false;
    _canRecruit = false;
    _canBuyGuns = true;
    _canTute = true;
};
if (!isNil { _civ getVariable "employee" }) then {
    _canSellDrugs = false;
    _canRecruit = false;
    _canBuyGuns = false;
};
if (_civ getVariable ["notalk", false]) then {
    _canSellDrugs = false;
    _canRecruit = false;
    _canBuyGuns = false;
};
...
if (_civ getVariable ["criminal", false]) then {
    _canSellDrugs = true;
    _canRecruit = false;
    _canBuyGuns = false;
    _canMission = false;
...
if (_delivery isNotEqualTo []) then {
    ...
    _options pushBack [
        format ["Deliver %1 x %2", _numitems, _itemcls call OT_fnc_weaponGetName],
        {
            ...
            private _stock = player call OT_fnc_unitStock;
            ...
            if (_found) then {
                [
                    player,
                    _civ,
                    ["I have a delivery for you", selectRandom ["About time!", "OK, thanks", "Sweet, thanks"]],
                    {
                        params ["_civ", "_itemcls", "_numitems"];
                        private _count = 0;
...
if (_isMayor && !(server_nosave getVariable [(_town + "transformerpaid"), true])) then {
    _options pushBack [
        format ["Donate 4000$"],
        OT_fnc_donateTransformer
    ];
};

if (_canRecruit) then {
    _options pushBack [
        format ["Recruit Civilian (-$%1)", _civprice],
        OT_fnc_recruitCiv
    ];
};
...
if (_canGangJob) then {
    ...
    if (_gangid > -1) then {
        ...
        if (_gang isNotEqualTo []) then {
            ...
            _options pushBack [
                format ["Do you want to join the resistance?"],
                {
...
if (_canMission) then {
    ...
    _options pushBack [
        format ["Buy Gear"],
        {
            ...
            private _gear = spawner getVariable [format ["facweapons%1", _faction], []];
            private _s = [];
            {
    ...
    _options pushBack [
        format ["Buy Blueprints"],
        {
            ...
            private _gear = spawner getVariable [format ["facvehicles%1", _faction], []];
            private _s = [];
            private _blueprints = server getVariable ["GEURblueprints", []];

            {
                if !(_x in _blueprints) then {
                    private _cost = cost getVariable [_x, [100, 0, 0, 0]];
                    private _req = 0;
                    private _base = _cost select 0;
        ...
        "Do you know any gangs nearby?",
        {
            ...
            if (_gangid > -1) then {
                ...
                if (_support > 50) then {
                    ...
                    _code = {
                        params ["", "_gangid", "_gang"];
                        private _town = player call OT_fnc_nearestTown;
                        private _mrkid = format ["gang%1", _town];
                ...
                private _go = {
                    _this spawn {
                        ...
                        if (_money < _cost) then {
                            ...
                            private _veh = OT_vehType_ferry createVehicle _pos;
...
if (_canSell) then {
    _options pushBack [
        "Sell",
        {
            ...
            private _cat = _civ getVariable "OT_shopCategory";
            private _categorystock = [player, _cat] call OT_fnc_unitStock;
...
if (_canSellDrugs) then {
    {
        ...
        if (_x in items player && !(_civ getVariable ["OT_askedDrugs", false])) then {
            ...
            private _drugname = _x call OT_fnc_weaponGetName;
            _options pushBack [
                format ["Sell %1", _drugname],
                {
                    OT_drugSelling = _this;
                    private _drugcls = _this;
                    private _drugname = _drugcls call OT_fnc_weaponGetName;
