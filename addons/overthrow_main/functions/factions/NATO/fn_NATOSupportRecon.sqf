...
private _abandoned = server getVariable ["NATOabandoned", []];
{
    ...
    if (([_pos, _posTarget] call OT_fnc_regionIsConnected) && !(_name in _abandoned)) then {
        private _d = (_pos distance _posTarget);
        if (_d < _dist) then {
            _dist = _d;
            _close = _pos;
        };
...
if (_isAir) then {
    ...
    private _spawnpos = _close findEmptyPosition [15, 100, OT_NATO_Vehicle_CTRGTransport];
    if (_spawnpos isEqualTo []) then { _spawnpos = _close findEmptyPosition [8, 100, OT_NATO_Vehicle_CTRGTransport] };
    private _veh = OT_NATO_Vehicle_CTRGTransport createVehicle _spawnpos;
    ...
    private _spawnpos = _convoypos findEmptyPosition [10, 100, OT_NATO_Vehicle_Transport_Light];
    if (_spawnpos isEqualTo []) then { _spawnpos = _convoypos findEmptyPosition [0, 100, OT_NATO_Vehicle_Transport_Light] };
    private _veh = OT_NATO_Vehicle_Transport_Light createVehicle _spawnpos;
    _group addVehicle _veh;

    private _wp = _group addWaypoint [_posTarget, 700];
...
private _wp = _group addWaypoint [_posTarget, 0];
