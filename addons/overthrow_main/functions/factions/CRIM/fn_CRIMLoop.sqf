...
if (crim_counter >= 12) then {
    ...
    {
        private _nogang = spawner getVariable [format ["nogang%1", _x], 0];
        private _stability = server getVariable [format ["stability%1", _x], 0];
        private _garrison = (server getVariable [format ["police%1", _x], 0]) + (server getVariable [format ["garrison%1", _x], 0]);
        private _town = _x;
