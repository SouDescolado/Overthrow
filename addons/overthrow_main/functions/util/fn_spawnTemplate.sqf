...
if (isNil { templates getVariable _type }) then {
    private _tpl = getText (configFile >> "CfgVehicles" >> _type >> "ot_template");
    ...
    private _template = call compile _tpl;
...
private _template = templates getVariable typeOf (_building);
...
private _vdir = vectorDir _building;
private _DCM = _building call OT_fnc_rotationMatrix;
private _objects = [];
//_objects = [_buildingpos,_dir,_template,0] call BIS_fnc_ObjectsMapper;

{
    _x params ["_type", "_rel", "_dir"];
    ...
    private _pos = [(_buildingpos select 0) + (_rel select 0), (_buildingpos select 1) + (_rel select 1), (_buildingpos select 2) + (_rel select 2)];

    if (_type != "Site_Minefield") then {
        private _o = createVehicle [_type, [0, 0, 0], [], 0, "CAN_COLLIDE"];
        _o setPosATL _pos;
        _dir = _dir + _bdir;
        if (_dir > 359) then {
            _dir = _dir - 360;
