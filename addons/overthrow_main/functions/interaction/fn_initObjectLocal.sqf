...
if (_currentObjectType == OT_item_Storage) then {
    ...
    _this addAction [
        ...
        true,
        "",
        "call OT_fnc_playerIsAtWarehouse"
    ];
    _this addAction [
        "Store In Warehouse",
        {
    ...
    if (_this call OT_fnc_playerIsOwner) then {
        _this addAction [
            ...
            true,
            "",
            "!(_target getVariable ['OT_locked', false])"
        ];
        _this addAction [
            "Unlock",
            {
        ...
        _this addAction [
            ...
            true,
            "",
            "(call OT_fnc_playerIsAtWarehouse && !((nearestObject [_target, OT_warehouse]) getVariable ['is_shared', false]))"
        ];
    };
    _this enableDynamicSimulation true;
};
